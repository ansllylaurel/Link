...
Процедура Готово(Команда)
	...
	Попытка
		ВыполнитьГотово(ПараметрыСессии, Команда);
	Исключение
...
Процедура ВыгрузитьКодОбработчиков(Команда)
	...
	Попытка
		ВыполнитьВыгрузитьКодОбработчиков(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Функция ЗарегистрироватьНовуюСессию(ИмяКоманды)
 ...
 ПараметрыСессии.Вставить("Механизм",     Механ
&НаКлиенте
Процедура ВыполнитьГотово(ПараметрыСессии, Команда)
	...
	Если Отказ Тогда
		Предупрежд
&НаКлиенте
Процедура ВыполнитьВыгрузитьКодОбработчиков(ПараметрыСессии, Команда)
...
КонецПроцедуры
ение(НСтр("ru = 'Не все необходимые шаги выполнены:'") + Символы.ПС + Символы.ПС + СтрокаСообщения); 
		Возврат;
	КонецЕсли;
