...
Процедура ТестПодключения(Команда)
	...
	Попытка
		ВыполнитьТестПодключения(ПараметрыСессии, Команда);
	Исключение
...
Процедура ПолучитьИнформациюОФайлеОбмена(Команда)
	...
	Попытка
		ВыполнитьПолучитьИнформациюОФайлеОбмена(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура УдалениеОтметитьВсе(Команда)
	...
	Попытка
		ВыполнитьУдалениеОтметитьВсе(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура УдалениеОтменитьВсе(Команда)
	...
	Попытка
		ВыполнитьУдалениеОтменитьВсе(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура УдалениеУдалить(Команда)
	...
	Попытка
		ВыполнитьУдалениеУдалить(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура ВыгрузкаОтметитьВсе(Команда)
	...
	Попытка
		ВыполнитьВыгрузкаОтметитьВсе(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура ВыгрузкаОтменитьВсе(Команда)
	...
	Попытка
		ВыполнитьВыгрузкаОтменитьВсе(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура ВыгрузкаУстановитьУзелОбмена(Команда)
	...
	Попытка
		ВыполнитьВыгрузкаУстановитьУзелОбмена(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура НастройкаОтладкиВыгрузки(Команда)
	...
	Попытка
		ВыполнитьНастройкаОтладкиВыгрузки(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура НаКлиенте(Команда)
	...
	Попытка
		ВыполнитьНаКлиенте(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура НаСервере(Команда)
	...
	Попытка
		ВыполнитьНаСервере(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура НастройкаОтладкиЗагрузки(Команда)
	...
	Попытка
		ВыполнитьНастройкаОтладкиЗагрузки(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Попытка
		ВыполнитьПрочитатьПравилаОбмена(ПараметрыСессии, Команда);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьВыгрузкаОтменитьВсе(ПараметрыСессии, Команда)
...
&НаКлиенте
Процедура ВыполнитьВыгрузкаОтметитьВсе(ПараметрыСессии, Команда)
	Для Каждого Строка из Объект.ТаблицаПравилВыгрузки.ПолучитьЭлементы() Цикл
		Строка.Включить = 1;
		УстановитьПометкиПодчиненных(Строка, "Включить");
	КонецЦикла;
...
&НаКлиенте
Процедура ВыполнитьВыгрузкаУстановитьУзелОбмена(ПараметрыСессии, Команда)
...
&НаКлиенте
Процедура ВыполнитьНаКлиенте(ПараметрыСессии, Команда)
...
&НаКлиенте
Процедура ВыполнитьНаСервере(ПараметрыСессии, Команда)
	Если ЭтоКлиент Тогда
...
&НаКлиенте
Процедура ВыполнитьНастройкаОтладкиВыгрузки(ПараметрыСессии, Команда)
...
&НаКлиенте
Процедура ВыполнитьНастройкаОтладкиЗагрузки(ПараметрыСессии, Команда)
	#Если ВебКлиент Тогда
		Предупреждение(НСтр("ru = 'Возможности отладки недоступны при использовании Веб-клиента'"));
		Возврат;
	#КонецЕсли
...
&НаКлиенте
Процедура ВыполнитьПолучитьИнформациюОФайлеОбмена(ПараметрыСессии, Команда)
	АдресФайла = "";
...
&НаКлиенте
Процедура ВыполнитьТестПодключения(ПараметрыСессии, Команда)
...
&НаКлиенте
Процедура ВыполнитьУдалениеОтменитьВсе(ПараметрыСессии, Команда)
...
&НаКлиенте
Процедура ВыполнитьУдалениеОтметитьВсе(ПараметрыСессии, Команда)
	Для Каждого Строка из УдаляемыеДанные.ПолучитьЭлементы() Цикл
...
&НаКлиенте
Процедура ВыполнитьУдалениеУдалить(ПараметрыСессии, Команда)
