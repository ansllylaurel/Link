...
Перем КодировкаТекст Экспорт; // MID-76. Переменная содержит кодировку текста записи лога.
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	РегистрационныеДанные = РаботаСДополнительнымиОтчетамиИОбработками.ЗаполнитьРегистрационныеДанные(
		"ДополнительнаяОбработка", "Универсальный обмен данными в формате XML", "1.02", "", Истина);
...
Процедура ИнициализироватьВедениеПротоколаОбмена() Экспорт
	...
	Если НЕ ЗначениеЗаполнено(КодировкаТекст) Тогда //MID-76
		мФайлПротоколаДанных = Новый ЗаписьТекста(ИмяФайлаПротоколаОбмена, КодировкаТекста.ANSI, , ДописыватьДанныеВПротоколОбмена);
	Иначе
		мФайлПротоколаДанных = Новый ЗаписьТекста(ИмяФайлаПротоколаОбмена, КодировкаТекст, , ДописыватьДанныеВПротоколОбмена);
	КонецЕсли; 
...
Функция ВыполнитьВыгрузкуИзмененныхДанныхДляУзлаОбмена(УзелОбмена, МассивПравилКонвертации, СтруктураДляУдаленияРегистрацииИзменений)
	...
	Попытка
		...
		МассивВыгружаемыхДанных_Аванкор	= Новый Массив();//Аванкор
		...
		Для Каждого СтрокаПравилаВыгрузки Из ВременныйМассивПравилКонвертации Цикл
			...
			Пока ВыборкаИзменений.Следующий() Цикл
				...
				МассивВыгружаемыхДанных_Аванкор.Добавить(Данные);
		...
		//Пока ВыборкаИзменений.Следующий() Цикл
		Для каждого ЭлементМассива из МассивВыгружаемыхДанных_Аванкор Цикл
...
Процедура ЗаполнитьСписокТиповДоступныхДляУдаления(ТаблицаДанных) Экспорт
	...
	Для каждого ОбъектМД Из Метаданные.РегистрыСведений Цикл
		...
		Если Не ПравоДоступа("Редактирование", ОбъектМД) Тогда
			Продолжить;
		КонецЕсли;
