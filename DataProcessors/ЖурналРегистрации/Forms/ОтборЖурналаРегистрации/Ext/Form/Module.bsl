...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не РаботаСКоллекциями.СпискиЗначенийИдентичны(СобытияПоУмолчанию, События) Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ВыборЗначенийЭлементовОтбораЖурналаРегистрации" И Источник = ЭтотОбъект Тогда
...
Процедура УстановитьОтборИЗакрытьФорму(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("УстановитьОтборИЗакрытьФорму");
	Попытка
		ВыполнитьУстановитьОтборИЗакрытьФорму(ПараметрыСессии);
	Исключение
...
&НаКлиенте
Процедура ВыполнитьУстановитьОтборИЗакрытьФорму(ПараметрыСессии)
	
	Если ПроверитьЗаполнениеКлиент() Тогда
		ОповеститьОВыборе(
...
Процедура УстановитьФлажкиВажности(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("УстановитьФлажкиВажности");
	Попытка
		ВыполнитьУстановитьФлажкиВажности(ПараметрыСессии);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьУстановитьФлажкиВажности(ПараметрыСессии)
...
Процедура СнятьФлажкиВажности(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("СнятьФлажкиВажности");
	Попытка
		ВыполнитьСнятьФлажкиВажности(ПараметрыСессии);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьСнятьФлажкиВажности(ПараметрыСессии)
	Для Каждого ЭлементСписка Из Важность Цикл
...
Процедура УстановитьСтатусыТранзакции(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("УстановитьСтатусыТранзакции");
	Попытка
		ВыполнитьУстановитьСтатусыТранзакции(ПараметрыСессии);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьУстановитьСтатусыТранзакции(ПараметрыСессии)
...
Процедура СнятьСтатусыТранзакции(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("СнятьСтатусыТранзакции");
	Попытка
		ВыполнитьСнятьСтатусыТранзакции(ПараметрыСессии);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьСнятьСтатусыТранзакции(ПараметрыСессии)
	Для Каждого ЭлементСписка Из СтатусТранзакции Цикл
		ЭлементСписка.Пометка = Ложь;
	КонецЦикла;
КонецПроцедуры
...
Функция ПроверитьЗаполнениеКлиент()
	...
	Если ЗначениеЗаполнено(ИнтервалОтбораДатаНачала)
		И ЗначениеЗаполнено(ИнтервалОтбораДатаОкончания)
		И ИнтервалОтбораДатаНачала > ИнтервалОтбораДатаОкончания Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Некорректно заданы условия отбора журнала регистрации. Дата начала больше даты окончания.",, "ИнтервалОтбораДатаНачала",, Отказ);
	КонецЕсли;
