...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ РаботаСПользователями.ЭтоПолноправныйПользователь(, Истина) Тогда
	...
	ТекстНеОпределено = РаботаСРегламентнымиЗаданиямиСлужебный.ТекстНеОпределено();
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("ВыполнитьРегламентноеЗаданиеВручную");
	Попытка
		ВыполнитьВыполнитьРегламентноеЗаданиеВручную(ПараметрыСессии);
	Исключение
...
&НаКлиенте
Процедура ВыполнитьВыполнитьРегламентноеЗаданиеВручную(ПараметрыСессии)
	...
	Если Элементы.ТаблицаРегламентныеЗадания.ТекущиеДанные = Неопределено Тогда
		...
		ПараметрыСессии.ТекстОшибки = НСтр("ru = 'Выберите регламентное задание.'");
		Возврат;
	КонецЕсли;
	...
	Для каждого ВыделеннаяСтрока Из ВыделенныеСтроки Цикл
		...
		КлючТекущейСтрокиРегламентныхЗаданий = РаботаСФормамиКлиент.ПолучитьКлючТекущейСтроки(Элементы.ТаблицаРегламентныеЗадания, "Наименование");
		
		ЗапускВыполнен = ВыполнитьРегламентноеЗаданиеВручнуюНаСервере(
		...
		РаботаСФормамиКлиент.ВосстановитьТекущуюСтрокуПоКлючу(ЭтаФорма.ТаблицаРегламентныеЗадания, Элементы.ТаблицаРегламентныеЗадания, "Наименование", КлючТекущейСтрокиРегламентныхЗаданий);
				
		Если ЗапускВыполнен Тогда
			
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Запущена процедура регламентного задания'"),
				,
				РаботаСоСтрокамиКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.
			...
			Если ЭтаФорма.ОбновлятьСписокАвтоматически Тогда
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = РаботаСоСтрокамиКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ОбновитьДанные(Команда)
	...
	Попытка
		ВыполнитьОбновитьДанные(ПараметрыСессии);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьОбновитьДанные(ПараметрыСессии)
...
Процедура ОткрытьРасписание(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("ОткрытьРасписание");
	Попытка
		ВыполнитьОткрытьРасписание(ПараметрыСессии);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьОткрытьРасписание(ПараметрыСессии)
...
Процедура ВключитьРегламентноеЗадание(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("ВключитьРегламентноеЗадание");
	Попытка
...
Процедура ВыключитьРегламентноеЗадание(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("ВыключитьРегламентноеЗадание");
	Попытка
...
Процедура ОткрытьФоновоеЗаданиеНаКлиенте(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("ОткрытьФоновоеЗаданиеНаКлиенте");
	Попытка
		ОткрытьФоновоеЗадание();
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
...
Процедура ОтменитьФоновоеЗадание(Команда)
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессию("ОтменитьФоновоеЗадание");
	Попытка
		ВыполнитьОтменитьФоновоеЗадание(ПараметрыСессии);
	Исключение
		ПараметрыСессии.ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
		Возврат;
...
&НаКлиенте
Процедура ВыполнитьОтменитьФоновоеЗадание(ПараметрыСессии)
	...
	Иначе
		...
		ТекстСообщения	= "";
		ЗаданиеОтменено	= ОтменитьФоновоеЗаданиеНаСервере(Элементы.ТаблицаФоновыеЗадания.ТекущиеДанные.Идентификатор, ТекстСообщения);
		
		Если ЗаданиеОтменено Тогда
			
			ТекстСообщения =
				"Задание отменено, но состояние отмены будет
		...
		ИначеЕсли Не ЗначениеЗаполнено(ТекстСообщения) Тогда
			...
			ТекстСообщения = "Не удалось отменить задание!";
...
Функция ПолучитьРасписание(Знач ИдентификаторРегламентногоЗадания)
	...
	Возврат РаботаСРегламентнымиЗаданиями.ПолучитьРасписаниеРегламентногоЗадания(
...
Процедура УстановитьРасписание(Знач ИдентификаторРегламентногоЗадания, Знач Расписание)
	...
	РаботаСРегламентнымиЗаданиями.УстановитьРасписаниеРегламентногоЗадания(
		ИдентификаторРегламентногоЗадания,
		Расписание);
...
Процедура СообщитьОбОкончанииРучногоВыполненияРегламентногоЗадания()
	...
	Если ЭтаФорма.ОбновлятьСписокАвтоматически И ИдентификаторыФоновыхЗаданийПриРучномВыполнении.Количество() > 0 Тогда
		
		ПодключитьОбработчикОжидания(
			"СообщитьОбОкончанииРучногоВыполненияРегламентногоЗадания", 10, Истина);
	КонецЕсли;
...
Функция ВыполнитьРегламентноеЗаданиеВручнуюНаСервере(Знач ИдентификаторРегламентногоЗадания,
	...
	ЗапускВыполнен = РаботаСРегламентнымиЗаданиямиСлужебный.ВыполнитьРегламентноеЗаданиеВручную(
		ИдентификаторРегламентногоЗадания,
		МоментЗапуска,
		ИдентификаторФоновогоЗадания,
		МоментОкончания,
...
&НаСервере
Функция ОтменитьФоновоеЗаданиеНаСервере(Знач Идентификатор, ТекстОшибки = Неопределено)
	
	ЗаданиеОтменено = РаботаСРегламентнымиЗаданиямиСлужебный.ОтменитьФоновоеЗадание(Идентификатор, Ложь, ТекстОшибки);
	
	Если ЗаданиеОтменено Тогда
		ОбновитьДанныеНаСервере();
	КонецЕсли;
	
	Возврат ЗаданиеОтменено;
...
Процедура УдалитьРегламентноеЗаданиеВыполнитьНаСервере(Знач Идентификатор)
	...
	Задание = РаботаСРегламентнымиЗаданиями.ПолучитьРегламентноеЗадание(Идентификатор);
...
Процедура ОбновитьТаблицуРегламентныхЗаданий(ИдентификаторРегламентногоЗадания = Неопределено)
	...
	Если РаботаСМетаданнымиПовтИсп.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ПодсистемаРаботаВМоделиСервиса = Метаданные.Подсистемы.Найти("СтандартныеПодсистемы").Подсистемы.Найти("РаботаВМоделиСервиса"); // "Найти" - чтобы расширенная проверка не ругалась
	Иначе
		ПодсистемаРаботаВМоделиСервиса = Неопределено;
	КонецЕсли;
	
	Если НЕ СистемныеФункции.РазделениеВключено() И ПодсистемаРаботаВМоделиСервиса <> Неопределено Тогда
	...
	Если ИдентификаторРегламентногоЗадания = Неопределено Тогда
		...
		Для каждого Задание ИЗ ТекущиеЗадания Цикл
			Если НЕ СистемныеФункции.РазделениеВключено()
...
Процедура ОбновитьСтрокуТаблицыРегламентныхЗаданий(Строка, Задание);
	...
	Строка.Наименование = РаботаСРегламентнымиЗаданиямиСлужебный.ПредставлениеРегламентногоЗадания(Задание);
	
	// Установка Даты завершения и Состояния завершения по последней фоновой процедуре
	СвойстваПоследнегоФоновогоЗадания = РаботаСРегламентнымиЗаданиямиСлужебный
...
Процедура ОбновитьТаблицуФоновыхЗаданий()
	...
	ТекущаяТаблица = РаботаСРегламентнымиЗаданиямиСлужебный.ПолучитьТаблицуСвойствФоновыхЗаданий(Отбор);
	...
	Для каждого Задание ИЗ ТекущаяТаблица Цикл
		...
		Обновляемое.СообщенияПользователюИОписаниеИнформацииОбОшибке 
			= РаботаСРегламентнымиЗаданиямиСлужебный.СообщенияИОписанияОшибокФоновогоЗадания(
...
Процедура УстановитьИспользованиеРегламентногоЗадания(Включено)
	...
	Для каждого ВыделеннаяСтрока Из Элементы.ТаблицаРегламентныеЗадания.ВыделенныеСтроки Цикл
		ТекущиеДанные = ТаблицаРегламентныеЗадания.НайтиПоИдентификатору(ВыделеннаяСтрока);
		Задание = РаботаСРегламентнымиЗаданиями.ПолучитьРегламентноеЗадание(ТекущиеДанные.Идентификатор);
