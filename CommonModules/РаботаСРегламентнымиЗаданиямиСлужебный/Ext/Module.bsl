...
Процедура ПриДобавленииОбработчиковСлужебныхСобытий(КлиентскиеОбработчики, СерверныеОбработчики) Экспорт
	...
	Если РаботаСМетаданнымиПовтИсп.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		СерверныеОбработчики["СтандартныеПодсистемы.РаботаВМоделиСервиса\ПриЗаполненииТаблицыПараметровИБ"].Добавить(
			"РаботаСРегламентнымиЗаданиямиСлужебный");
	КонецЕсли;
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	Если РаботаСМетаданнымиПовтИсп.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		МодульРаботаВМоделиСервиса = СистемныеФункции.ОбщийМодуль("РаботаВМоделиСервиса");
...
Функция ВыполнитьРегламентноеЗаданиеВручную(Знач Задание,
	...
	Задание = РаботаСРегламентнымиЗаданиями.ПолучитьРегламентноеЗадание(Задание);
	...
	Если СвойстваПоследнегоФоновогоЗадания <> Неопределено
	   И СвойстваПоследнегоФоновогоЗадания.Состояние = СостояниеФоновогоЗадания.Активно Тогда
		...
		КонецЕсли;
	Иначе
		НаименованиеФоновогоЗадания = РаботаСоСтрокамиКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Запуск вручную: %1'"), ПредставлениеРегламентногоЗадания(Задание));
...
//
// Параметры:
//  Идентификатор											- Строка - Строка уникального идентификатора ФоновогоЗадания
//  ВызыватьИсключениеПриОтсутствующемИлиНеактивномЗадании  - Булево - при Ложь вместо соответствующего исключения текст о нем будет помещен в ТекстОшибки
//  ТекстОшибки												- Строка - выходной параметр, заполняется текстом сообщения об ошибке
//
// Возвращаемое значение:
//	Отменено - Булево - истина, если задание удалось отменить
// 
Функция ОтменитьФоновоеЗадание(Идентификатор, ВызыватьИсключениеПриОтсутствующемИлиНеактивномЗадании = Истина, ТекстОшибки = Неопределено) Экспорт
	...
	Если МассивФоновыхЗаданий.Количество() = 1 Тогда
		...
		Если ФоновоеЗадание.Состояние <> СостояниеФоновогоЗадания.Активно Тогда
			ТекстОшибки = НСтр("ru = 'Задание не выполняется, его нельзя отменить.'");
		КонецЕсли;
	Иначе
		ТекстОшибки = НСтр("ru = 'Фоновое задание не найдено на сервере.'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		Если ВызыватьИсключениеПриОтсутствующемИлиНеактивномЗадании Тогда
...
Процедура ДобавитьСвойстваФоновыхЗаданий(Знач МассивФоновыхЗаданий, Знач ТаблицаСвойствФоновыхЗаданий)
	...
	Пока Индекс >= 0 Цикл
		...
		Если РегламентноеЗадание = Неопределено
		   И РаботаСоСтрокамиКлиентСервер.ЭтоУникальныйИдентификатор(ФоновоеЗадание.Ключ) Тогда
