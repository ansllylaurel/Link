...
Функция ПерваяФорма_ПолучитьПараметрыПодключения(Сервер, Порт = 8080) Экспорт
	
	Источник = "РаботаСВнешнимиСистемами.ПерваяФорма_УстановитьСоединение()";
	
	ПараметрыПодключения = Новый Структура("СоединениеHTTP, ЗаголовкиHTTP", Новый HTTPСоединение(Сервер + ".g1.lan", Порт,,,,, Новый ЗащищенноеСоединениеOpenSSL()), Новый Соответствие);
	ПараметрыПодключения.ЗаголовкиHTTP.Вставить("Content-type", "application/json");
	
	ЗаписьJSON = Новый ЗаписьJSON;
	ЗаписьJSON.УстановитьСтроку(Новый ПараметрыЗаписиJSON(ПереносСтрокJSON.Нет));
	
	ЗаписатьJSON(ЗаписьJSON, Новый Структура("login, password", "UK_UFO_Robot", "UK_UFO_Robot"));
	
    ТелоЗапроса = ЗаписьJSON.Закрыть();
	
	ТекстСообщения = "Не удалось установить соединение с 1-ой Формой! Сервер [" + Сервер + ".g1.lan]"; 
	
	HTTPОтвет = HTTP_ВыполнитьЗапрос("POST", ПараметрыПодключения, "/api/auth/token-v2", ТелоЗапроса, ТекстСообщения);
	
	Если HTTPОтвет = Неопределено Тогда
		
		ОбработатьОшибку(ТекстСообщения, Источник);
		Возврат Неопределено;
...
Функция ЕСИА_Финанс_ПараметрыПодключения(ContentType = "application/json") Экспорт
	
	ПараметрыПодключения = Новый Структура();    
	
	Если ОбщегоНазначения.ЗначениеПараметраСеанса("ЭтоРабочаяИнформационнаяБаза") Тогда
		ПараметрыПодключения.Вставить("СоединениеHTTP", Новый HTTPСоединение("lk.region-am.ru",,,,,, Новый ЗащищенноеСоединениеOpenSSL));
	Иначе
