...
#Область ШаблонЗапросаЮрЛиц
	ШаблонЗапросаЮрЛиц	= "
		...
		|	Data_Source nvarchar(15),
		|	Partner_ID nvarchar(9),
		|	Name_Clean nvarchar(500),
		|	Name nvarchar(500),
		|	Name_Full_Clean nvarchar(500),
		|	Name_Full nvarchar(500),
		|	INN nvarchar(12),
...
Функция ПолучитьДанныеБухгалтерииОВознагражденииУК(ВидВознаграждения, НачалоПериода, КонецПериода, Организация, СчетаДоверительногоУправления = Неопределено, СоединениеСБухгалтерией = Неопределено) Экспорт
	...
	// ============================================================================
	// ВРЕМЕННАЯ ДИАГНОСТИКА: пропадают ЦК-договоры по виду «Управление активами
	// целевых капиталов». Цель — увидеть, на каком именно шаге функции теряется
	// конкретный ЦК-договор «БДФ ВИКТОРИЯ» (230823-1/DU/UL, 23.08.2023).
	// После выяснения причины этот блок надо удалить.
	// ============================================================================
	#Область ВРЕМЕННАЯ_ДИАГНОСТИКА_ЦК
	
	НомерЦКДляДиагностики = "230823-1/DU/UL";
	
	ВсеТаблицы = Новый Массив;
	ВсеТаблицы.Добавить("_ВознагражденияУК");
	ВсеТаблицы.Добавить("_ДоговорыСчетовДУ");
	ВсеТаблицы.Добавить("_ВознагражденияУКСоСчетамиДУ");
	ВсеТаблицы.Добавить("_ОшибкиИдентификации");
	ВсеТаблицы.Добавить("_ТаблицаВознаграждений");
	
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("==== ДИАГНОСТИКА ЦК (" + НомерЦКДляДиагностики + ") ====");
	
	Для Каждого ИмяТаблицы Из ВсеТаблицы Цикл
		Попытка
			Дамп = Запрос.МенеджерВременныхТаблиц.Таблицы[ИмяТаблицы].ПолучитьДанные().Выгрузить();
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ИмяТаблицы + ": не удалось получить — " + ОписаниеОшибки());
			Продолжить;
		КонецПопытки;
		
		КолонкаНомера = Дамп.Колонки.Найти("НомерДоговора");
		Совпадения = 0;
		Если КолонкаНомера <> Неопределено Тогда
			Для Каждого СтрокаДампа Из Дамп Цикл
				Если Найти(Строка(СтрокаДампа.НомерДоговора), НомерЦКДляДиагностики) > 0 Тогда
					Совпадения = Совпадения + 1;
					
					Свойства = Новый Массив;
					Для Каждого Колонка Из Дамп.Колонки Цикл
						Свойства.Добавить(Колонка.Имя + "=" + Строка(СтрокаДампа[Колонка.Имя]));
					КонецЦикла;
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ИмяТаблицы + ": " + СтрСоединить(Свойства, " | "));
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			ИмяТаблицы + " — всего строк: " + Дамп.Количество()
			+ "; колонка НомерДоговора: " + ?(КолонкаНомера = Неопределено, "нет", "есть")
			+ "; совпадений по " + НомерЦКДляДиагностики + ": " + Совпадения);
	КонецЦикла;
	
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю("==== КОНЕЦ ДИАГНОСТИКИ ЦК ====");
...
Функция ПрочитатьАнкетныеДанныеИзФайла(ОбъектXDTO, ВнешняяСистема)
	...
	// Общее
	ДокументОбъект.ПометкаУдаления				= Ложь;
	ДокументОбъект.ИсточникАнкеты				= Перечисления.ИсточникиАнкетИЗаявок.Агент;
...
#Область ЗаписьДанных
	...
	Запрос.Parameters.Append(Запрос.CreateParameter("@Name"					, 200	, 1, 500));
	...
	Запрос.Parameters.Append(Запрос.CreateParameter("@INN"					, 200	, 1, 12));
	Запрос.Parameters.Append(Запрос.CreateParameter("@Period"				, 135	, 1, 8));
	Запрос.Parameters.Append(Запрос.CreateParameter("@LegalAddress"			, 200	, 1, 500));
	Запрос.Parameters.Append(Запрос.CreateParameter("@PlaceOfBirth"			, 200	, 1, 500));
	Запрос.Parameters.Append(Запрос.CreateParameter("@Name_Clean"			, 200	, 1, 500));
...
Функция ПерваяФорма_ПолучитьПараметрыПодключения(Сервер, Порт = 8080, Логин = "", Пароль = "") Экспорт
	...
	Иначе
		...
		ПараметрыПодключения.ЗаголовкиHTTP.Вставить("Cookie", HTTPОтвет.Заголовки["Set-Cookie"]);
