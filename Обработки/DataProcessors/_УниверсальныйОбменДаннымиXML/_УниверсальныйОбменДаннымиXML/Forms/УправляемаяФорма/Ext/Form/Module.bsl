...
Процедура ВыполнитьНастройкаОтладкиВыгрузки(ПараметрыСессии)
	...
	Если Не ПравилаЗагружены Тогда
		...
		ТекстОшибки = НСтр("ru = 'Не прочитаны правила обмена'");
		ДополнитьПараметрыВыполненияСессии(ПараметрыСессии, ТекстОшибки);
		Предупреждение(ТекстОшибки);
		Возврат;
...
Процедура ВыполнитьПрочитатьПравилаОбмена(ПараметрыСессии)
	...
	Иначе
		...
		Если ПустаяСтрока(Объект.ИмяФайлаПравилОбмена) Тогда
			ТекстОшибки = НСтр("ru = 'Не задан файл правил обмена'");
			ДополнитьПараметрыВыполненияСессии(ПараметрыСессии, ТекстОшибки);
			Предупреждение(ТекстОшибки);
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуИзФормы(ПараметрыСессии = Неопределено)
	...
	Иначе
		...
		Если ПустаяСтрока(ИмяФайлаОбмена) Тогда
			ТекстОшибки = НСтр("ru = 'Не задан файл обмена данными на сервере'");
			ДополнитьПараметрыВыполненияСессии(ПараметрыСессии, ТекстОшибки);
			Предупреждение(ТекстОшибки);
			Возврат;
		КонецЕсли;
...
Процедура ВыполнитьВыгрузкуИзФормы(ПараметрыСессии = Неопределено)
	...
	Если НЕ Объект.НепосредственноеЧтениеВИБПриемнике и Не ЭтоКлиент Тогда
		...
		Если Врег(СокрЛП(Объект.ИмяФайлаПравилОбмена)) = Врег(СокрЛП(Объект.ИмяФайлаОбмена)) Тогда
			...
			ТекстОшибки = НСтр("ru = 'Файл правил обмена совпадает с файлом данных'");
			ДополнитьПараметрыВыполненияСессии(ПараметрыСессии, ТекстОшибки);
			Предупреждение(ТекстОшибки);
			Возврат;
