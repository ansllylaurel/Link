...
Процедура КомандаОчиститьВыполнить()
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессиюНаСервере("КомандаОчиститьВыполнить");
...
Процедура КомандаЗаполнитьСписокВалютВыполнить()
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессиюНаСервере("КомандаЗаполнитьСписокВалютВыполнить");
	ЗаполнитьВалюты();
	ЗарегистрироватьОкончаниеСессии(ПараметрыСессии);
...
Процедура КомандаПодборВалютВыполнить()
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессиюНаСервере("КомандаПодборВалютВыполнить");
...
Процедура ЗагрузитьКурсыВалют()
	...
	ПараметрыСессии = ЗарегистрироватьНовуюСессиюНаСервере("ЗагрузитьКурсыВалют");
...
Процедура ВыполнитьЗагрузкуКурсовВалют(ПараметрыСессии)
	...
	СтатусОперации = ЗагрузитьКурсыСРБК(ПараметрыСессии);
...
&НаКлиенте
Функция ЗагрузитьКурсыСРБК(ПараметрыСессии)
	...
	Для Каждого СостояниеЗагрузки Из Результат Цикл
		...
		Иначе
			...
			ТекстОшибки = СостояниеЗагрузки.Сообщение;
			ДополнитьПараметрыВыполненияСессии(ПараметрыСессии, ТекстОшибки);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки,, ИмяПоля, "Объект");
		КонецЕсли;
	КонецЦикла;
