...
Процедура СохранитьПарольИПродолжитьВыполнить()
	...
	Если Пароль <> ПодтверждениеПароля Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Пароль и подтверждение пароля не совпадают'"),, "Пароль");
		Возврат;
	КонецЕсли;
...
Процедура СохранитьПароль(Значение)
	...
	РаботаСХранилищем.ХранилищеОбщихНастроекСохранить(
...
Функция ЗагрузитьПароль()
	...
	Возврат РаботаСХранилищем.ХранилищеОбщихНастроекЗагрузить("ФормаПодтвержденияПароляУчетнойЗаписи", УчетнаяЗапись);
