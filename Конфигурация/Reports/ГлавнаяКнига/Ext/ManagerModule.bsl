...
Функция СформироватьЗаголовок(ПараметрыОтчета)
	...
	ОписаниеПериода = РаботаСБухгалтерскимиОтчетамиКлиентСервер.ПолучитьПредставлениеПериода(ПараметрыОтчета.НачалоПериода, ПараметрыОтчета.КонецПериода);
...
Функция СформироватьПодписи(ПараметрыОтчета) Экспорт
	...
	//	
	//ДанныеОтветственногоЛица = РаботаСБухгалтерскимиОтчетами.ПолучитьДанныеОтветственногоЛица(ДополнительныеПараметры);
...
Функция ПолучитьТекстОбщегоЗапроса(ПараметрыОтчета)
	...
	|	NULL КАК СуммаНачКт
	|ИЗ
	|	РегистрБухгалтерии.БухгалтерскийУчет.Обороты(&НачалоПериода, &КонецПериода, " + ПараметрыОтчета.ПредставленияПериодичности[ПараметрыОтчета.Периодичность] + ", , , " + СтрокаОграниченийПоРеквизитам + ", , ) КАК Обороты
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Основной КАК Счета
	|			ПО Обороты.Счет = Счета.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Основной КАК КорСчета
	...
	|ИЗ
	|	РегистрБухгалтерии.БухгалтерскийУчет.Остатки(&НачалоПериода, , , " + СтрокаОграниченийПоРеквизитам + ") КАК Остатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Основной КАК Счета
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.УчетныйРегистр) Тогда
		Если ПараметрыОтчета.УчетныйРегистр <> "БухгалтерскийУчет" Тогда
			Текст = СтрЗаменить(Текст, "РегистрБухгалтерии.БухгалтерскийУчет", "РегистрБухгалтерии." + ПараметрыОтчета.УчетныйРегистр);
		КонецЕсли;
		
		//Иванов 2016-08-25 Начало...
		Если ПараметрыОтчета.УчетныйРегистр = "ЕдиныйБухгалтерскийУчет" Тогда
			Текст = СтрЗаменить(Текст, "ПланСчетов.Основной", "ПланСчетов.Единый");
		Иначе
			Текст = СтрЗаменить(Текст, "ПланСчетов.Единый", "ПланСчетов.Основной");
		КонецЕсли;
	...
	Если СтруктураПараметров.РазвернутоеСальдо Тогда
		...
		Иначе	
			Если ЗначениеЗаполнено(СтруктураПараметров.СтрокаРазвернутогоСальдо.ПоСубконто) Тогда
				...
				ДанныеСчета = БухгалтерскийУчет.ПолучитьСвойстваСчета(СтруктураПараметров.Счет);
		...
		+ ТекстПолей + "
		|ИЗ
		|	РегистрБухгалтерии.БухгалтерскийУчет.ОстаткиИОбороты(&НачалоПериода, &КонецПериода, " + ПараметрыОтчета.ПредставленияПериодичности[ПараметрыОтчета.Периодичность] + ", , Счет В (&Субсчета), , " + СтрокаОграниченийПоРеквизитам + ") КАК ОстаткиИОбороты
		|
		...
		Если ЗначениеЗаполнено(ПараметрыОтчета.УчетныйРегистр) Тогда
			Если ПараметрыОтчета.УчетныйРегистр <> "БухгалтерскийУчет" Тогда
				ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "БухгалтерскийУчет", ПараметрыОтчета.УчетныйРегистр);
			КонецЕсли;
		КонецЕсли;
		...
		Запрос.УстановитьПараметр("Организация", ПараметрыОтчета.Организация);
		Запрос.УстановитьПараметр("Субсчета",    БухгалтерскийУчет.СчетаВИерархии(СтруктураПараметров.Счет));
