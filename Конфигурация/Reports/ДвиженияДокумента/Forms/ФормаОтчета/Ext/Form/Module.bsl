...
Функция ПолучитьДанныеПоДвижениям(МетаданныеРегистра, ВидРегистра)
	...
	Для Счетчик = 1 По 2 Цикл
		...
		Для Каждого Измерение Из МетаданныеРегистра[ИмяКоллекции] Цикл
			...
			Если (ВидРегистра = "Бухгалтерии")И(Счетчик = 1)И(НЕ Измерение.Балансовый) Тогда
				
				ИмяТаблицы.Колонки.Добавить(Измерение.Имя + "Дт", , Измерение.Синоним + "Дебет");

				ТекстПоля = " ПРЕДСТАВЛЕНИЕССЫЛКИ(Таблица." + Измерение.Имя + "Дт ) КАК " + Измерение.Имя +"Дт, ";

				СтруктураПолей[ИмяКоллекции].СтрокаПолей = СтруктураПолей[ИмяКоллекции].СтрокаПолей
								+ ?(ПустаяСтрока(СтруктураПолей[ИмяКоллекции].СтрокаПолей),"", ",") + Измерение.Имя + "Дт";

				ТекстПолейВыборки = ТекстПолейВыборки + Символы.ПС + ТекстПоля;
				
				
				ИмяТаблицы.Колонки.Добавить(Измерение.Имя + "Кт", , Измерение.Синоним + "Кредит");

				ТекстПоля = " ПРЕДСТАВЛЕНИЕССЫЛКИ(Таблица." + Измерение.Имя + "Кт ) КАК " + Измерение.Имя +"Кт, ";

				СтруктураПолей[ИмяКоллекции].СтрокаПолей = СтруктураПолей[ИмяКоллекции].СтрокаПолей
								+ ?(ПустаяСтрока(СтруктураПолей[ИмяКоллекции].СтрокаПолей),"", ",") + Измерение.Имя + "Кт";
	...
	Для Каждого Ресурс Из МетаданныеРегистра[ИмяКоллекции] Цикл
		...
		Если (ВидРегистра = "Бухгалтерии")И(НЕ Ресурс.Балансовый) Тогда
			
			ТекстПолейВыборки = ТекстПолейВыборки + "
								|Таблица." + Ресурс.Имя + "Дт КАК " + Ресурс.Имя +"Дт, ";

			СтруктураПолей[ИмяКоллекции].СтрокаПолей = СтруктураПолей[ИмяКоллекции].СтрокаПолей
							+ ?(ПустаяСтрока(СтруктураПолей[ИмяКоллекции].СтрокаПолей),"", ",") + Ресурс.Имя + "Дт";

			ТаблицаРесурсов.Колонки.Добавить(Ресурс.Имя + "Дт", , Ресурс.Синоним + "Дебет");
			...
			СтруктураПолей[ИмяКоллекции].СтрокаПолей = СтруктураПолей[ИмяКоллекции].СтрокаПолей
							+ ?(ПустаяСтрока(СтруктураПолей[ИмяКоллекции].СтрокаПолей),"", ",") + Ресурс.Имя + "Кт";

			ТаблицаРесурсов.Колонки.Добавить(Ресурс.Имя + "Кт", , Ресурс.Синоним + "Кредит");
...
Процедура СформироватьОтчет()
	...
	Для каждого РегистрБухгалтерии Из Метаданные.РегистрыБухгалтерии Цикл
		ИсключаемыеРегистры.Добавить(РегистрБухгалтерии.Имя);
	КонецЦикла;
	
	МетаданныеДокумента = Отчет.Документ.Метаданные();
	МассивРегистров     = ПолучитьМассивИспользуемыхРегистров(Отчет.Документ, МетаданныеДокумента.Движения, ИсключаемыеРегистры);
	...
	РаботаСОтчетами.ЗаполнитьНастройкиПечати(ТабличныйДокумент, РеквизитФормыВЗначение("Отчет").Метаданные().ПолноеИмя());	
КонецПроцедуры
